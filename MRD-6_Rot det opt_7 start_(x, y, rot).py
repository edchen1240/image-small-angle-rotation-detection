...
#M2PCS.batch_file_relocate(dir_img_folder, list_keywords_select=list_keywords_select_to_dump, clean_dump_folder = False)
...
#[3] Read datasheets
list_of_dfs, list_of_sheetnames = M1PCS.read_excel_with_multiple_sheets_into_list_of_dfs(path_excel)
...
for cnt, iter_bsn in enumerate(list_bsn):
    ...
    dic_hyperp = {
        ...
        'scope_penalty': 0,                                     # [Keep 0]This doesn't help with the optimization. Set to 0, not even 1. (0)
        'eps': 1e-2,                                            # [Keep 1e-2] Gradiant increase if this value reduces. # eps < 1e-3 grad = 0. (1e-2)
        'num_epochs': 100,                                      # Usually 100 epoch. (50)
        ...
        'power_of': 2,                                          # [Keep 2] 2 = MSE, 4 = MQE. (2)
        'grad_cal_selection': 'grad_ctr_diff',                  # grad_fwd_diff = forward difference, grad_ctr_diff = center difference.
        'rot_eps_constraint': 5e-4,                             # rot is more sensitive, so it might need smaller eps. (5e-4)
        'rot_lr_constraint': 5e-4,                              # 1 Means no constraint, typically use 1e-4 ~ 1e-5 if necessary. (1e-4)
        ...
        'AO_beta1': 0.99,                                        # Decrease AO_beta1 if (x,y) oscillates. (0.99)
        'AO_beta2':0.9999,                                       # Decrease beta2 will result in faster adaption and more reactive or noisey update. (0.999)
        'AO_eps': 1e-6,                                         # To avoid dividied by zero. Not important acutally.
        'probe_ratio': 1.05,                                # (Less than but close to 1) Algorithm tend to find futher COR with smaller AOR. This force it to try closer COR with larger AOR
    ...
    probe_ratio = dic_hyperp['probe_ratio']
    ...
    #[33] Before optimization start, use initial valuse to perform overlap check.
    ...
    """
    Start real training (optimization) process
    """
    #[41] 0th Train to minimize substraction loss
    name_tag = 'p0 (1/7)'
    (df_train_recrod_xyrot_0, text_log) \
          = M3TRA.train_x_y_rot_to_minimize_substraction_loss_eff(arrimg_t2, arrimg_t1,
                                                                init_shift_0,
                                                                dic_hyperp,
                                                                text_log, name_tag)
          
    #[42] Recover df_train_recrod
    (list_ss_x, list_ss_y, list_ss_r,
            list_grad_x, list_grad_y, list_grad_r,
            list_x, list_y, list_r,
            list_loss, list_loss_eff, list_eff_pixel), idx_lowest_loss_eff= M3TRA.recover_lists_from_df_train_recrod_xyrot(df_train_recrod_xyrot_0)
    print(f'\n\n[list_loss_eff]_1\n{[int(x) for x in list_loss_eff]}\n\n')
    
    #[43] Extract optimized values of 0th train
    i_0opt = idx_lowest_loss_eff
    x_0opt, y_0opt, r_0opt = list_x[i_0opt], list_y[i_0opt], list_r[i_0opt]
          
          
          
    #[44] Train to minimize substraction loss (extend by 1 probe_ratio)
    name_tag = 'p1 (2/7)'
    init_shift_p1 = [x_0opt * probe_ratio, y_0opt * probe_ratio, r_0opt / probe_ratio]
    (df_train_recrod_xyrot_p1, text_log) \
          = M3TRA.train_x_y_rot_to_minimize_substraction_loss_eff(arrimg_t2, arrimg_t1,
                                                                init_shift_p1,
                                                                dic_hyperp,
                                                                text_log, name_tag)
          
    #[48] Train to minimize substraction loss (extend by 2 probe_ratio)
    name_tag = 'p2 (3/7)'
    init_shift_p2 = [x_0opt * (probe_ratio**2), y_0opt * (probe_ratio**2), r_0opt / (probe_ratio**2)]
    (df_train_recrod_xyrot_p2, text_log) \
          = M3TRA.train_x_y_rot_to_minimize_substraction_loss_eff(arrimg_t2, arrimg_t1,
                                                                init_shift_p2,
                                                                dic_hyperp,
                                                                text_log, name_tag)
    ...
    #[49] Train to minimize substraction loss (extend by 2 probe_ratio)
    name_tag = 'p3 (4/7)'
    init_shift_p3 = [x_0opt * (probe_ratio** 3), y_0opt * (probe_ratio** 3), r_0opt / (probe_ratio** 3)]
    (df_train_recrod_xyrot_p3, text_log) \
          = M3TRA.train_x_y_rot_to_minimize_substraction_loss_eff(arrimg_t2, arrimg_t1,
                                                                init_shift_p3,
                                                                dic_hyperp,
                                                                text_log, name_tag)
    ...
    #[50] Train to minimize substraction loss (reduce by 1 probe_ratio)
    name_tag = 'n1 (5/7)'
    init_shift_n1 = [x_0opt / probe_ratio, y_0opt / probe_ratio, r_0opt * probe_ratio]
    (df_train_recrod_xyrot_n1, text_log) \
          = M3TRA.train_x_y_rot_to_minimize_substraction_loss_eff(arrimg_t2, arrimg_t1,
                                                                init_shift_n1,
                                                                dic_hyperp,
                                                                text_log, name_tag)
          
    #[52] Train to minimize substraction loss (reduce by 2 probe_ratio)
    name_tag = 'n2 (6/7)'
    init_shift_n2 = [x_0opt / (probe_ratio**2), y_0opt / (probe_ratio**2), r_0opt * (probe_ratio**2)]
    (df_train_recrod_xyrot_n2, text_log) \
          = M3TRA.train_x_y_rot_to_minimize_substraction_loss_eff(arrimg_t2, arrimg_t1,
                                                                init_shift_n2,
                                                                dic_hyperp,
                                                                text_log, name_tag)
          
    #[53] Train to minimize substraction loss (extend by 2 probe_ratio)
    name_tag = 'n3 (7/7)'
    init_shift_n3 = [x_0opt / (probe_ratio** 3), y_0opt / (probe_ratio** 3), r_0opt * (probe_ratio** 3)]
    (df_train_recrod_xyrot_n3, text_log) \
          = M3TRA.train_x_y_rot_to_minimize_substraction_loss_eff(arrimg_t2, arrimg_t1,
                                                                init_shift_n3,
                                                                dic_hyperp,
                                                                text_log, name_tag)
          
    df_train_recrod_xyrot_all = pd.concat([df_train_recrod_xyrot_n3, 
                                             df_train_recrod_xyrot_n2, 
                                             df_train_recrod_xyrot_n1,
                                             df_train_recrod_xyrot_0,
                                             df_train_recrod_xyrot_p1,
                                             df_train_recrod_xyrot_p2,
                                             df_train_recrod_xyrot_p3], axis=0, ignore_index=True)
    ...
    """
    #[61] Recover df_train_recrod
    (list_ss_x, list_ss_y, list_ss_r,
            list_grad_x, list_grad_y, list_grad_r,
            list_x, list_y, list_r,
            list_loss, list_loss_eff, list_eff_pixel), idx_lowest_loss_eff= M3TRA.recover_lists_from_df_train_recrod_xyrot(df_train_recrod_xyrot_all)
    text_log += f'\n{tabulate(df_train_recrod_xyrot_all, headers="keys", tablefmt="orgtbl")}\n\n'
    print(f'\n\n[list_loss_eff]_1\n{[int(x) for x in list_loss_eff]}\n\n')


    #[62] Extract optimized values
    ...
    #[64] Check two opt lengthes
    len_n3_opt = len(df_train_recrod_xyrot_n3)
    len_n2_opt = len(df_train_recrod_xyrot_n2)
    len_n1_opt = len(df_train_recrod_xyrot_n1)
    len_0_opt = len(df_train_recrod_xyrot_0)
    len_p1_opt = len(df_train_recrod_xyrot_p1)
    len_p2_opt = len(df_train_recrod_xyrot_p2)
    len_p3_opt = len(df_train_recrod_xyrot_p3)
    len_combined_opt = len(df_train_recrod_xyrot_all)
    
    
    #[65] Organize list_marker_idx
    list_marker_idx.append(len_n3_opt + len_n2_opt + len_n1_opt) # No need to pluse 1 here.
    list_marker_idx.append(i_opt)
    text =  f'\n\n[list_marker_idx] {list_marker_idx}\n'\
            f'  length (n3, n2, n1, 0, p1, p2, p3): {len_n3_opt}, {len_n2_opt}, {len_n1_opt}, {len_0_opt}, {len_p1_opt}, {len_p2_opt}, {len_p3_opt}.'
    print(text)
    text_log += text
    ...
    text_note = f'[{timestamp}]_{iter_bsn}\n'\
                ...
                f'Epoch (set, ratio), probe_ratio:     {num_epochs},   {min_epoch_ratio}, {probe_ratio}\n'\
                f'length (n2, n1, 0, p1, p2): {len_n2_opt}, {len_n1_opt}, {len_0_opt}, {len_p1_opt}, {len_p2_opt}\n'\
                ...
                f'{AO_optimizer}:           {AO_beta1}, {AO_beta2}, {AO_eps}\n'\
